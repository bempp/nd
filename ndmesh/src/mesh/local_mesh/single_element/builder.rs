...
///
/// The following gives an example of creating a new mesh consisting
...
/// ```
#[derive(Debug)]
pub struct SingleElementMeshBuilder<T: Scalar> {
    gdim: usize,
...
impl<T: Scalar> GeometryBuilder for SingleElementMeshBuilder<T> {
    type MeshGeometry = SingleElementGeometry<T, CiarletElement<T, IdentityMap, T>>;
    fn create_geometry(
        &self,
        point_ids: &[usize],
        coordinates: &[Self::T],
...
impl<T: Scalar> TopologyBuilder for SingleElementMeshBuilder<T> {
    type MeshTopology = SingleTypeTopology;
    fn create_topology(
        &self,
...
impl<T: Scalar> MeshBuilder for SingleElementMeshBuilder<T> {
