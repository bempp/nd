...
use crate::{
    ...
    traits::{Builder, GeometryBuilder, MeshBuilder, TopologyBuilder},
    types::Scalar,
};
use itertools::izip;
use ndelement::{
...
/// Mesh builder for a mesh with a mixture of element types
#[derive(Debug)]
pub struct MixedMeshBuilder<T: Scalar> {
    gdim: usize,
...
impl<T: Scalar> MixedMeshBuilder<T> {
...
impl<T: Scalar> Builder for MixedMeshBuilder<T> {
    type Mesh = MixedMesh<T, CiarletElement<T, IdentityMap, T>>;
    type T = T;
    ...
    fn add_cell(&mut self, id: usize, cell_data: (ReferenceCellType, usize, &[usize])) {
        ...
        if reference_cell::dim(cell_data.0) != reference_cell::dim(self.cell_types[0]) {
            panic!("Cannot create mesh with cells of different topological dimensions");
        }
    ...
    fn create_mesh(&self) -> MixedMesh<T, CiarletElement<T, IdentityMap, T>> {
        let cell_vertices =
        ...
        // We finally sort the vector to make sure that mesh generation is reproducible across runs
        ...
        self.create_mesh_from_topology_geometry(topology, geometry)
    }
...
impl<T: Scalar> GeometryBuilder for MixedMeshBuilder<T> {
    type MeshGeometry = MixedGeometry<T, CiarletElement<T, IdentityMap, T>>;
    fn create_geometry(
        &self,
        point_ids: &[usize],
        coordinates: &[Self::T],
...
impl<T: Scalar> TopologyBuilder for MixedMeshBuilder<T> {
    type MeshTopology = MixedTopology;
    fn create_topology(
        &self,
...
impl<T: Scalar> MeshBuilder for MixedMeshBuilder<T> {
    fn create_mesh_from_topology_geometry(
        ...
        MixedMesh::new(topology, geometry)
    }
}
