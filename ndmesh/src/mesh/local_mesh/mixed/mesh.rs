...
impl<'e, T: Scalar, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>>
    MixedMeshEntity<'e, T, E>
{
    /// Create new
    pub fn new(
        mesh: &'e MixedMesh<T, E>,
        cell_type: ReferenceCellType,
        cell_index: usize,
        entity_type: ReferenceCellType,
        entity_index: usize,
        ...
        Self {
            ...
            entity_type,
            entity_index,
            geometry_element_index: mesh.geometry.insertion_indices_to_element_indices
                [mesh.topology.insertion_indices[&cell_type][cell_index]],
            geometry_cell_index: mesh.geometry.insertion_indices_to_cell_indices
                [mesh.topology.insertion_indices[&cell_type][cell_index]],
        }
    }
}
impl<T: Scalar, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>> Entity
    for MixedMeshEntity<'_, T, E>
{
    type T = T;
    ...
    fn local_index(&self) -> usize {
        self.mesh.topology.cell_entity_index(
            self.cell_type,
            self.cell_index,
            self.entity_type,
            self.entity_index,
    ...
    fn topology(&self) -> Self::Topology<'_> {
        MixedEntityTopology::new(&self.mesh.topology, self.entity_type(), self.local_index())
    }
...
/// Mixed mesh entity iterator
#[derive(Debug)]
pub struct MixedMeshEntityIter<
    'a,
    T: Scalar,
    E: MappedFiniteElement<CellType = ReferenceCellType, T = T>,
> {
...
impl<'a, T: Scalar, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>>
    MixedMeshEntityIter<'a, T, E>
{
    /// Create new
    pub fn new(mesh: &'a MixedMesh<T, E>, entity_type: ReferenceCellType) -> Self {
        Self {
            mesh,
            entity_type,
            index: 0,
...
impl<'a, T: Scalar, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>> Iterator
    for MixedMeshEntityIter<'a, T, E>
{
    type Item = MixedMeshEntity<'a, T, E>;

    fn next(&mut self) -> Option<MixedMeshEntity<'a, T, E>> {
        self.index += 1;
        self.mesh.entity(self.entity_type, self.index - 1)
    }
}

/// Serial mixed element mesh
#[derive(Debug)]
pub struct MixedMesh<T: Scalar, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>> {
...
impl<T: Scalar + serde::Serialize> ConvertToSerializable
    for MixedMesh<T, CiarletElement<T, IdentityMap, T>>
{
...
impl<T: Scalar, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>> MixedMesh<T, E> {
    /// Create new
...
impl<T: Scalar> MixedMesh<T, CiarletElement<T, IdentityMap, T>> {
    ...
    for MixedMesh<T, E>
{
            ...
            if dim == self.topology_dim() {
                ...
                for t in &self.topology.entity_types()[self.topology_dim()] {
                        ...
                        self.topology.upward_connectivity[&entity_type][t][local_index].first()
                    {
                        ...
                        {
                            return Some(MixedMeshEntity::new(self, *t, *cell, entity_type, index));
                        }
                    }
                }
                None
    ...
    fn entity_iter(&self, entity_type: ReferenceCellType) -> Self::EntityIter<'_> {
        MixedMeshEntityIter::new(self, entity_type)
    }
...
impl<T: Scalar + Equivalence, E: MappedFiniteElement<CellType = ReferenceCellType, T = T>>
    DistributableMesh for MixedMesh<T, E>
{
    type ParallelMesh<'a, C: Communicator + 'a> =
        ParallelMeshImpl<'a, C, MixedMesh<T, CiarletElement<T, IdentityMap, T>>>;
    ...
    fn distribute<'a, C: Communicator>(
        ...
        let mut b = MixedMeshBuilder::<T>::new(self.geometry.dim());
...
mod test {
    ...
    fn example_mesh_mixed() -> MixedMesh<f64, CiarletElement<f64, IdentityMap, f64>> {
    ...
    fn test_geometry_map() {
        ...
        let map = mesh.geometry_map(ReferenceCellType::Triangle, 1, &pts);
        ...
        pts[[0, 1]] = 1.0;
        pts[[1, 1]] = 1.0;
        let map = mesh.geometry_map(ReferenceCellType::Quadrilateral, 1, &pts);
