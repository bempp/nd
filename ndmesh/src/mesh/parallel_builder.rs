...
use super::ParallelMeshImpl;
use crate::{
    traits::{
        Builder, Entity, GeometryBuilder, Mesh, MeshBuilder, ParallelBuilder, Topology,
        TopologyBuilder,
    },
    ...
    B::Mesh: Sync,
{
    ...
    fn create_parallel_mesh_root<'a, C: Communicator>(
        ...
        // If we have only a single process we can just create a serial mesh.
        if comm.size() == 1 {
            let serial_mesh = self.create_mesh();
            ...
            for dim in 0..self.tdim() {
                for t in serial_mesh.entity_types(dim) {
                    let entity_count = serial_mesh.entity_iter(*t).count();
                    owners.insert(*t, vec![Ownership::Owned; entity_count]);
                    global_indices.insert(*t, (0..entity_count).collect_vec());
                }
            }

            return ParallelMeshImpl::new(comm, serial_mesh, owners, global_indices);
        }
        // Partition the cells
        ...
        // Note that the array is of the size of the points in the mesh and contains garbage information
        ...
        // This is executed on all ranks and creates the local mesh.
        self.create_parallel_mesh_internal(
            comm,
            point_indices,
            coordinates,
            vertex_indices,
...
trait ParallelBuilderFunctions: Builder + GeometryBuilder + TopologyBuilder + MeshBuilder {
    ...
    /// Internal function to create parallel mesh
    #[allow(clippy::too_many_arguments)]
    fn create_parallel_mesh_internal<'a, C: Communicator>(
        &self,
        comm: &'a C,
    ...
    ) -> ParallelMeshImpl<'a, C, Self::Mesh>
    where
        Self::Mesh: Sync,
    {
        ...
        // Everything is properly sorted now. Can generate the local meshes.
        ...
        let serial_mesh = self.create_mesh_from_topology_geometry(topology, geometry);
        ...
        // The global indices are not necessarily identical to the original mesh indices.
        ...
        let point_type = serial_mesh.entity_types(0)[0];
        let mut owners = HashMap::new();
        ...
        for dim in 1..self.tdim() {
            for t in serial_mesh.entity_types(dim) {
                ...
                // We get out the chunk length by probing the first entity. This only works because the mesh has a single
                // element type.
                let chunk_length = serial_mesh
                    .entity(*t, 0)
                    .unwrap()
                    .topology()
                ...
                let number_of_entities = serial_mesh.entity_iter(*t).count();
                ...
                for entity in serial_mesh.entity_iter(*t) {
        ...
        ParallelMeshImpl::new(comm, serial_mesh, owners, global_indices)
    }
    ...
    fn assign_vertex_owners(&self, nprocesses: usize, cell_owners: &[usize]) -> Vec<usize> {
        ...
        // for the points of the meshes that are not vertices.
        vertex_owners
    }
...
impl<B: Builder + GeometryBuilder + TopologyBuilder + MeshBuilder> ParallelBuilderFunctions for B
where
    Vec<B::T>: Buffer,
    B::T: Equivalence,
    Vec<B::EntityDescriptor>: Buffer,
