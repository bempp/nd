...
use crate::traits::{Builder, Entity, Geometry, GmshExport, GmshImport, Mesh, Point};
use itertools::izip;
...
/// Convert gmsh element node ordering to ndmesh ordering
fn permute_gmsh_to_ndmesh(
...
impl<G: Mesh<EntityDescriptor = ReferenceCellType>> GmshExport for G {
    ...
    fn import_from_gmsh_v1(&mut self, s: String) {
        ...
        for _ in 0..num_elements {
            ...
            let node_tags = node_tags
                ...
                .collect::<Vec<_>>();
            let (cell, cell_type, degree) = permute_gmsh_to_ndmesh(element_type, &node_tags);
                ...
                "$Elements" => {
                    ...
                    for _ in 0..num_elements {
                        ...
                        let (cell, cell_type, degree) =
                            permute_gmsh_to_ndmesh(elm_type, &node_tags);
                        self.add_cell_from_nodes_and_type(tag, &cell, cell_type, degree);
                    }
                    line.clear();
                }
                ...
                "$Elements" => {
                    ...
                    for _ in 0..num_entity_blocks {
                        ...
                        for _ in 0..num_elements_in_block {
                            ...
                            let (cell, cell_type, degree) =
                                permute_gmsh_to_ndmesh(element_type, &node_tags);
                            self.add_cell_from_nodes_and_type(tag, &cell, cell_type, degree);
                        }
                    }
                    line.clear();
...
mod test {
    use super::*;
    use crate::{
        MixedMeshBuilder, SingleElementMeshBuilder,
    ...
    fn test_regular_sphere_gmsh_io() {
        let g = regular_sphere::<f64>(2, ReferenceCellType::Triangle, 1);
        g.export_as_gmsh("_test_io_sphere.msh");

        let g = regular_sphere::<f64>(2, ReferenceCellType::Quadrilateral, 1);
    ...
    fn test_export_tetrahedra() {
        let mut b = SingleElementMeshBuilder::<f64>::new(3, (ReferenceCellType::Tetrahedron, 1));
        b.add_point(0, &[0.0, 0.0, 0.0]);
        b.add_point(1, &[1.0, 0.0, 0.0]);
        b.add_point(2, &[0.0, 1.0, 0.0]);
        b.add_point(3, &[1.0, 1.0, 0.0]);
    ...
    #[test]
    fn test_hexahedra() {
        let mut b = SingleElementMeshBuilder::<f64>::new(3, (ReferenceCellType::Hexahedron, 1));
        b.add_point(0, &[0.0, 0.0, 0.0]);
        b.add_point(1, &[1.0, 0.0, 0.0]);
        b.add_point(2, &[0.0, 1.0, 0.0]);
        b.add_point(3, &[1.0, 1.0, 0.0]);
    ...
    fn test_high_order_triangles() {
        let mut b = SingleElementMeshBuilder::<f64>::new(3, (ReferenceCellType::Triangle, 5));
        b.add_point(0, &[0.0, 0.0, 0.0]);
    ...
    fn test_import_mixed_triangle() {
        let mut b = MixedMeshBuilder::<f64>::new(3);
    ...
    fn test_import_wrong_degree() {
        let mut b = SingleElementMeshBuilder::<f64>::new(3, (ReferenceCellType::Triangle, 2));
        b.import_from_gmsh("meshes/sphere_triangle.msh");
        let _g = b.create_mesh();
    }
