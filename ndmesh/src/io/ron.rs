//! RON I/O
use crate::traits::{ConvertToSerializable, Mesh, RONExport, RONImport};
#[cfg(feature = "mpi")]
use crate::traits::{ParallelMesh, RONExportParallel};
#[cfg(feature = "mpi")]
use mpi::traits::Communicator;

impl<G: Mesh + ConvertToSerializable> RONExport for G {
    fn to_ron_string(&self) -> String {
        ron::to_string(&self.to_serializable()).unwrap()
    }
}

impl<G: Mesh + ConvertToSerializable> RONImport for G
where
...
#[cfg(feature = "mpi")]
impl<'a, C: Communicator + 'a, G: ParallelMesh<C = C>> RONExportParallel<'a, C> for G
where
...
mod test {
    use super::*;
    use crate::{
        MixedMesh, MixedMeshBuilder, SingleElementMesh,
    ...
    fn test_ron_export_and_import() {
        let g = regular_sphere::<f64>(1, ReferenceCellType::Triangle, 1);
    ...
    fn test_ron_export_and_import_mixed() {
        ...
        let g2 = MixedMesh::<f64, CiarletElement<f64, IdentityMap, f64>>::import_from_ron(
